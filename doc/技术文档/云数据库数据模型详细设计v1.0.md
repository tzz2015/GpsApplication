...
| 阶段 | 做什么 | 说明 |
|------|--------|------|
| **第一步** | **在 AGC 控制台建立云数据库** | 按本文「三、对象类型详细定义」在 Cloud DB 中创建对象类型：User、Track、TrackPoint、TombSpot、Post、Feedback；发布后客户端才能按同一模型读写（V1.0 不再使用 TrackSegment）。 |
...
| Feedback | Preference 或 RDB 表，字段与 Cloud DB 一致 | 意见反馈；V1.0 可先仅存本地（如 Preferences 的 feedback_list），需后台汇总时再同步到 Cloud DB。 |

**要点**：本地表结构（字段名、类型）与云库对象类型保持一致，上传时即可按条/按批映射写入 Cloud DB，无需再做复杂转换。

### 4. 开发前期你需要完成的事（小结）

1. **在 AGC 控制台**：创建 Cloud DB，按本文第三节创建并发布 **User、Track、TrackPoint、TombSpot、Post、Feedback** 共 6 个对象类型及索引（V1.0 不使用 TrackSegment）。  
2. **在工程内**：  
...
| 5 | Post | 打卡点图文内容 | ✅ 是 |
| 6 | Feedback | 意见反馈（用户提交的反馈内容与联系方式） | ✅ 是 |
| 7 | Comment | 评论 | ❌ V2.0 |
...
| 字段名 | 类型 | 主键 | 非空 | 默认值 | 说明 |
|--------|------|:----:|:----:|--------|------|
| feedbackId | String | ✅ | ✅ | — | 反馈唯一 ID（建议客户端生成，如 `fb_` + 时间戳） |
| userId | String | — | — | — | 提交人 userId，关联 User.userId；未登录可为空 |
| content | Text | — | ✅ | — | 反馈内容（必填） |
| contact | String | — | — | — | 联系方式（选填，如邮箱或手机号，便于回复） |
| createdAt | Date | — | — | — | 提交时间 |

**索引建议**：主键 `feedbackId`；可按 `createdAt` 倒序建索引，便于后台按时间查看反馈列表。

**说明**：用于「意见反馈」功能，用户提交的问题或建议写入本表；V1.0 客户端当前可将反馈先存本地（Preferences），若需云端汇总与后台查看，需在 AGC 控制台创建本对象类型并在客户端实现上传逻辑。
...
User (userId) ──1:N──→ Feedback (userId)  意见反馈（可选同步云端）
```
...
2. 在 **对象类型** 中依次新建：**User、Track、TrackPoint、TombSpot、Post、Feedback**（Comment、FamilyMember 留 V2.0）。V1.0 不使用 TrackSegment。
...
4. **Feedback**：若启用意见反馈云端存储，需在 schema.json 中增加 Feedback 对象类型，字段含 feedbackId、userId、content、contact、createdAt，与本文 3.8 节一致。
