...
- **客户端技术栈**：
  ...
  - 地图与定位：优先选择华为地图 SDK（HMS Core Map & Location），支持 2D/3D、**图层切换**（标准/卫星/地形）、海拔/高度展示。
  - 本地存储：鸿蒙首选 `Preference` / `RDB` / Local KV，用于轨迹本地缓存与离线数据。**GPS 数据本地优先存储**，网络良好时再同步到云端。
- **后端能力（基于华为 AppGallery Connect，BaaS 方案）**：
  - 认证（AGC Auth）：V1.0 仅支持鸿蒙授权登录（AGC 与华为账号体系打通）；手机验证码登录及其他第三方登录不做。**首次授权登录的用户自动在 User 表中创建记录。**
...
## 二、系统整体架构
- **客户端层（鸿蒙 App）**
  - **首页双 Tab 结构**：Tab1「我的轨迹」展示当前用户创建的轨迹列表；Tab2「公开轨迹」展示所有可见范围为公开的轨迹列表（Cloud DB 查询 `visibility = PUBLIC`，按创建时间倒序）。两 Tab 共用轨迹详情页，根据是否为创建人决定是否展示编辑/删除等能力。
  - 负责 UI 展示、轨迹记录（GPS + 海拔）、权限控制的前端校验。
  - 通过 AGC SDK 直接读写 Cloud DB / Cloud Storage。
  - 提供**GPS 数据本地优先存储**功能（核心策略）：
    - 轨迹点 GPS 信息（经纬度、海拔、时间戳等）在采集时**必须先写入本地存储**（鸿蒙 RDB）；
    - 网络良好时自动将未同步数据上传到云端（Cloud DB / Cloud Storage）；
    - 无网时所有数据仅操作本地，保证山上记录不丢失。
- **云后端层（AGC BaaS）**
  - **Auth**：统一身份认证和会话管理（V1.0 仅支持鸿蒙授权登录），为 App 提供用户身份信息和 Token。**首次登录的用户自动在 Cloud DB User 表中创建记录。**
...
- **location 模块**：负责所有**定位相关**逻辑——定位权限、持续/单次定位、轨迹点采样（经纬度、海拔、时间戳）、轨迹点序列缓存（**本地优先写入 RDB**）；对外提供接口或组件供 entry 的轨迹记录页调用。
- **map 模块**：负责所有**地图相关**逻辑——地图初始化、2D/3D、**图层切换**（标准地图/卫星地图/地形地图）、轨迹线（polyline）绘制、墓地点标记、当前位置、地图截图等；对外提供地图组件供 entry 的轨迹记录页、轨迹详情页嵌入。
...
- **GPS 数据本地优先存储策略（核心）**：
  - 所有轨迹点 GPS 信息在采集时**必须先写入本地存储**（鸿蒙 RDB），不依赖网络状态。
  - 本地数据库中每条记录增加 `syncStatus`（pending / synced）字段，标记是否已同步到云端。
  - 网络良好时，自动将 `syncStatus = pending` 的数据批量上传到 Cloud DB，上传成功后标记为 synced。
  - 无网或弱网时仅操作本地，保证山上记录不中断、不丢失。
- 轨迹记录策略：
...
- **上传时机（建议）**：
  - 设置打卡点 → 尝试将新增 TrackPoint 及该 TombSpot 上传（若有网）；
...
- 以 Cloud DB 的实体（Entity）建模：
  - `UserEntity`：用户基本信息。**首次鸿蒙授权登录后自动创建记录。**
  ...
  - `TombSpotEntity`：打卡点（墓地点）信息（关联 Track、顺序索引、名称、经纬度、海拔等；对应轨迹上的每个墓地）。**创建人可修改打卡点名称。**
...
- 实现：
  ...
  - **图层切换**（标准地图/卫星地图/地形地图）使用地图 SDK 的 `setMapType` 或同类接口，用户可在地图控件区选择所需图层；
...
- **首次登录自动注册用户（User 表）**：
  - 鸿蒙授权登录成功后，客户端检查 Cloud DB User 表中是否已有当前用户的记录（查询 `userId == AGC Auth UID`）。
  - **若不存在**：自动创建一条 User 记录，userId 取 AGC Auth 返回的 UID，昵称和头像可从鸿蒙账号信息获取（若 API 支持），其余字段留空由用户后续完善。
  - **若已存在**：读取已有记录，更新本地缓存。
  - 此逻辑确保每个登录用户在用户列表中都有对应记录，后续轨迹的 `ownerUserId`、Post 的 `authorUserId` 等均可正确关联。
...
- `UserEntity`：userId、phone、nickname、avatarUrl、createdAt 等。**首次鸿蒙授权登录时自动创建。**
- `TrackEntity`：trackId、ownerUserId、title、description、visibility、status（editing/done）、startName、endName、createdAt 等。
- `TrackPointEntity`：pointId、trackId、orderIndex、lat、lng、altitude、timestamp 等（一条轨迹一条连续点序列，支持暂停/继续产生的间隙）。**本地存储增加 syncStatus 字段标记同步状态。**
- `TombSpotEntity`：tombId、trackId、orderIndex、name、personInfo、remark、lat、lng、altitude 等（打卡点即墓地点，按顺序关联轨迹）。**创建人可修改 name 字段（打卡点名称）。**
...
- **性能与稳定性**：
  ...
  - **GPS 数据本地优先存储**：确保弱网/无网环境下数据不丢失，网络恢复后自动同步。
