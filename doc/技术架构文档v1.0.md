# 轨迹 App 技术架构文档 V1.0（方案 A：鸿蒙 App + AGC BaaS）
...
- **后端能力（基于华为 AppGallery Connect，BaaS 方案）**：
  - 认证（AGC Auth）：手机号验证码登录 / 鸿蒙账号登录（AGC 与华为账号体系打通）；当前阶段不考虑其他第三方登录（如微信）。
...
- **云后端层（AGC BaaS）**
  - **Auth**：统一身份认证和会话管理（当前仅支持手机号验证码、鸿蒙账号），为 App 提供用户身份信息和 Token。
...
### 3.5 认证与登录（鸿蒙账号 + AGC Auth）
- **登录方式**：V1.0 仅支持手机号验证码、鸿蒙账号（华为账号）两种方式；均通过 AGC Auth 统一签发 Token，Cloud DB/Cloud Storage/Cloud Functions 校验该 Token。其他第三方登录（如微信）当前阶段不考虑。
- **鸿蒙账号与 AGC 打通**：
  ...
  - 首次通过鸿蒙账号登录时，AGC 会创建对应用户（或绑定到已有手机号账号，若在业务上做账号合并）；后续用同一鸿蒙账号登录即复用该 AGC 用户，保证 Cloud DB 中 `UserEntity` 与轨迹数据的归属一致。
