...
| **第三步** | **先实现「只写本地」** | 轨迹记录、打卡点（墓地点）、图文等先落本地库；无网或弱网时仅操作本地，保证山上记录不丢。 |
| **第四步** | **再实现「上传/同步」** | 网络恢复或合适时机（如设置打卡点、结束轨迹、进入首页时）将本地未同步数据上传到 Cloud DB / Cloud Storage。 |
...
- **上传时机（建议）**：  
  - 设置打卡点 → 将新增 TrackPoint 及该 TombSpot 上传；  
  - 结束轨迹 → 将 Track 状态改为 done，并上传/更新 Track（含起点名、终点名）；  
  - 保存打卡点图文 → 图片先传 Cloud Storage 拿 URL，再与 Post/TombSpot 一起写 Cloud DB；  
...
| Track | RDB 表，字段与 Cloud DB Track 一致 | 主键 trackId；含 startName、endName；增加字段如 `syncStatus`（pending/synced）便于上传队列。 |
| TrackPoint | RDB 表，字段与 Cloud DB 一致 | 直接关联 trackId，按 orderIndex 排序；一条轨迹一条连续点序列（暂停/继续会产生时间间隙）。 |
| TombSpot | RDB 表，字段与 Cloud DB 一致 | 直接关联 trackId，按 orderIndex 排序；每个打卡点即一个墓地点。 |
...
| 3 | TrackPoint | 轨迹点（直接关联 Track，GPS+海拔+顺序） | ✅ 是 |
| 4 | TombSpot | 打卡点（墓地点）信息（直接关联 Track，按顺序） | ✅ 是 |
| 5 | Post | 打卡点图文内容 | ✅ 是 |
| 6 | Comment | 评论 | ❌ V2.0 |
...
**说明**：一条轨迹仅一条连续路径，含起点、若干打卡点（墓地点）、终点；支持暂停/继续记录。V1.0 不再使用“轨迹段（Segment）”概念。
...
**说明**：一条轨迹一条连续点序列，直接关联 Track；暂停/继续会产生时间上的间隙，顺序仍按 orderIndex 排列。**数据量控制**：写入云库的轨迹点应为经「采集端采样 + 上传前抽稀」后的点集（详见《技术架构文档v1.0》3.1 节），单条轨迹建议将点数控制在合理范围（如数百至约一两千点），以降低存储与查询成本；可考虑批量写入、查询时按 trackId 拉取。
...
**说明**：V1.0 中每条轨迹有若干打卡点（TombSpot），每个打卡点即一个墓地点；打卡点详情页（P1-5）展示本表 + 该 tombId 下的 Post 列表。
...
User (userId)
  ...
  │           │
  │           ├── 1:N → TrackPoint (trackId)  一条轨迹一条连续点序列
  │           └── 1:N → TombSpot (trackId)    一条轨迹若干打卡点（墓地点）
  │
